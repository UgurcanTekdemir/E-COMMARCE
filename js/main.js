...
function addTocart(event) {
  //Sepete ekle butonuna tıkladığımızda sepete ekleyeceğim ürünün id'sine erişme
  const productID = parseInt(event.target.dataset.id);
  //Dizinin içerisinden tıkladığımız id'li elemanı bulmak için find methodunu kullandık
  const product = Products.find((product) => product.id === productID);

  if (product) {
    //Sepette tıkladığımız veri önceden var mı kontrol ederiz.
    const existingItem = cart.find((item) => item.id === productID);
    //Sepette tıkladığımız ürün varsa bu ürünün miktarını bir arttırırız
    if (existingItem) {
      existingItem.quantity++;
    } else {
      //Tıkladığımız ürün sepette yoksa yeni bir şeklinde ekleriz.
      const cartItem = {
        id: product.id,
        title: product.title,
        price: product.price,
...
//Cart dizisinden ve localStoragedan silmek ürünü sildik ve sayfayı günceledik
function removeFromCart(event) {
  const productID = parseInt(event.target.dataset.id);
  console.log(cart);
  //cart dizisinden silmek istediğimiz ürünü id'sine göre silme yapıldı.
...
//İnputun içerisindeki miktar değişince çalışacak fonsiyon
...
//LocalStorage'a veri eklemek için kullandık
...
function calculateCartTotal() {
  //reduce iki değer ister. Bunlardan birincisi içerisinde yapacağımız işlem, ikinci ise başlangıç değer
  const total = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
  cartTotalElement.textContent = `Total: $${total.toFixed(2)}`;
}

//Sayfa cart.html sayfasında renderCartItems fonsiyonun çalışması gerekiyor.
if (window.location.pathname.includes("cart.html")) {
  renderCartItems();
  calculateCartTotal();
} else {
  //Sayfa index.html sayfasında renderProduct fonsyonu çalışacak
  renderProducts();
}
